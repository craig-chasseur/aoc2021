...
int CountPaths(
    const absl::flat_hash_map<std::string, absl::flat_hash_set<std::string>>&
        edges,
    absl::flat_hash_set<std::string>& visited_small, bool double_small,
  ...
  for (const std::string& next : edges.at(current_cave)) {
    if (!visited_small.contains(next)) {
      ...
      paths += CountPaths(edges, visited_small, double_small, next);
      if (!absl::ascii_isupper(next.front())) visited_small.erase(next);
    } else if (!double_small && next != "start") {
      paths += CountPaths(edges, visited_small, true, next);
    }
  }
  return paths;
}
...
int main(int argc, char** argv) {
  ...
  std::cout << CountPaths(edges, visited_small, false, "start") << "\n";
