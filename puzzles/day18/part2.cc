...
int main(int argc, char** argv) {
  ...
  int max_magnitude = 0;
  for (auto outer_it = lines.begin(); outer_it != lines.end(); ++outer_it) {
    for (auto inner_it = lines.begin(); inner_it != lines.end(); ++inner_it) {
      ...
      result->Reduce();
      max_magnitude = std::max(max_magnitude, result->Magnitude());
